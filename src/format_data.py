...
def get_data_in_vectoral_format(emotion, pixels):
    ...
    x_public_test_vectoral = np.array(
        x_vectoral_data[PUBLIC_TEST_START:PUBLIC_TEST_END])
    ...
    y_public_test = np.array(
        y_data_categorical[PUBLIC_TEST_START:PUBLIC_TEST_END])
    ...
    return (x_train_vectoral, x_public_test_vectoral, x_private_test_vectoral,
            y_train, y_public_test,
            y_private_test)  # Returns data in vectoral format
...
def get_data_in_matrix_format(emotion, pixels):
    ...
    x_public_test_matrix = np.array(
        np_image_array[PUBLIC_TEST_START:PUBLIC_TEST_END])
    ...
    return (x_train_matrix, x_public_test_matrix, x_private_test_matrix,
            y_train, y_public_test,
            y_private_test)  # Returns data in matrix format
